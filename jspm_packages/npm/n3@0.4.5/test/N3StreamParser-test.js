...
describe('N3StreamParser', function() {
  ...
  describe('An N3StreamParser instance', function() {
    ...
    it("doesn't parse an invalid stream", shouldNotParse(['z.'], 'Syntax error: unexpected "z." on line 1.'));
    it('emits "prefix" events', shouldEmitPrefixes(['@prefix a: <IRIa>. a:a a:b a:c. @prefix b: <IRIb>.'], {
      a: 'IRIa',
      b: 'IRIb'
    }));
  });
...
function shouldEmitPrefixes(chunks, expectedPrefixes) {
  return function(done) {
    ...
    transform.on('prefix', function(prefix, iri) {
      prefixes[prefix] = iri;
    });
    transform.on('error', done);
