...
function is_integer(val) {
  return !isNaN(val) && isFinite(val) && val > -9007199254740992 && val < 9007199254740992 && Math.floor(val) === val;
}
class Literal {
  constructor(reasoner, literal) {
    ...
    if (lang)
      this[_language] = new LanguageTag(N3.getLiteralLanguage(literal)).toString();
    if (type)
...
class Node {
  ...
  is(type, callback) {
    ...
    var answer = (type == this.id || find(this, {
      subject: this.id,
      predicate: rdf.type,
      object: type
    }).length > 0 || find(this, {
      ...
      object: type
    }).length > 0 || filter(this.types(), (item) => {
      return item.is(type);
    }).length > 0 || filter(this.superClasses(), (item) => {
      return item.is(type);
    }).length > 0 || filter(this.superProperties(), (item) => {
      return item.is(type);
    }).length > 0);
...
class Reasoner {
  ...
  literal(val, options) {
    ...
    var lit = N3.createLiteral(String(val), options.language || options.type);
