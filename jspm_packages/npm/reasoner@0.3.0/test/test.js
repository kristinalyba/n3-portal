...
describe('It Works', function() {
  ...
  it('Should be reasonable...', function(done) {
    ...
    async.parallel([function(done) {
      ...
      types.forEach(function(item) {
        assert(item.id === 'http://example.org/bar' || item.id === 'http://example.org/baz');
      });
      done();
    }, function(done) {
      node.superClasses();
      ...
      types.forEach(function(item) {
        assert(item.id === 'http://example.org/boo' || item.id === 'http://example.org/aaa' || item.id === 'http://example.org/ccc');
      });
      done();
    }, function(done) {
      ...
      types.forEach(function(m) {
        assert(m.id === 'http://example.org/bbb' || m.id === 'http://example.org/boo' || m.id === 'http://example.org/bar' || m.id === 'http://example.org/foo');
      });
      done();
    }, function(done) {
      ...
      labels.forEach(function(label) {
        assert(label == 'testing' || label == 'testing2');
      });
      done();
    }], function() {
      done();
  ...
  it('should reduce properly', function(done) {
    ...
    var reasoner = new Reasoner(graph);
    var types = ['http://example.org/foo', 'http://example.org/bar', 'http://example.org/baz', 'http://example.org/baz', 'http://example.org/aaa'];
    var res = reasoner.reduce(types);
    assert.equal(res.length, 2);
    res.forEach(function(type) {
      assert(type === 'http://example.org/bar' || type === 'http://example.org/baz');
    });
    done();
  });
});
